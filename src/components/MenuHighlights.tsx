...
export default function MenuHighlights({ onOpenMenu }: MenuHighlightsProps) {
  ...
  return (
    <section
      ...
      className="relative h-[190vh] bg-zinc-950"
    >
      <div className="sticky top-0 h-screen overflow-hidden flex flex-col-reverse md:flex-row bg-zinc-900">
        ...
        <motion.h2
          ...
          className="absolute top-2 left-0 right-0 z-30 text-3xl font-serif text-primary md:hidden drop-shadow-md p-6"
        >
          Our Most <br />
        ...
        {/* Left Panel - Text */}
        <div className="w-full md:w-1/2 h-[45%] md:h-full flex items-center justify-center p-6 md:p-16 relative z-10">
          <div className="relative max-w-lg w-full flex flex-col justify-center h-full md:py-40">
            <motion.h2
              initial={{ opacity: 0, y: 20 }}
            ...
            <div className="relative flex-grow">
              {items.map((item, index) => (
                ...
                >
                  ...
                  {/* Review Section */}
                  <div className="mt-auto bg-white/5 backdrop-blur-md p-4 md:p-6 border border-white/10 rounded-xl relative">
                    ...
                    <p className="text-primary font-serif text-xs md:text-sm">— {item.review.author}</p>
                    <p className="text-primary font-serif text-[10px] md:text-xs">{item.review.source}</p>
                  </div>

                  <div className="mt-6 md:mt-12">
                    <button
                      onClick={onOpenMenu}
                      className="text-primary border-b border-primary pb-1 hover:text-white hover:border-white transition-colors text-sm md:text-base"
                    >
                      View Full Menu
                    </button>
                  </div>
        ...
        {/* Right Panel - Images */}
        <div className="w-full md:w-1/2 h-[45%] md:h-full relative overflow-hidden flex items-center justify-center z-10 -mb-4">
          {items.map((item, index) => (
            <motion.div
              key={item.id}
            ...
            >
              <div className="relative w-[280px] h-[280px] md:w-[600px] md:h-[600px] flex items-center justify-center">

                {/* Visual Plate/Border Background (Behind) */}
                <div className="absolute inset-0 m-auto w-[280px] h-[280px] md:w-[500px] md:h-[500px] xl:w-[600px] xl:h-[600px] rounded-full bg-black/40 border-[4px] md:border-[6px] border-primary/80 shadow-2xl backdrop-blur-sm blur-[2px] z-0" />
                ...
                <img
                  src={item.image}
                  alt={item.name}
                  className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 h-[135%] w-[135%] max-w-none z-10 object-contain drop-shadow-2xl"
                />
              </div>
            </motion.div>
          ))}
